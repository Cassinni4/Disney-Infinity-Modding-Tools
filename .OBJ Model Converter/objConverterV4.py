...
# Usage: python objConverterV4.py model.obj
...
with open(octFilename, "w", encoding="utf-8") as oct_File:
    ...
    EXT(f"[EXT] .OCT root -> ObjectDatabase start")

    # Material bundle pool (points to .mtb)
    oct_File.write('  <MaterialBundlePool>\n')
    oct_File.write(f'    <MaterialBundle name="{base}" file="{os.path.basename(mtbFilename)}"/>\n')
    oct_File.write('  </MaterialBundlePool>\n')
    print(".MTB entry written")
    EXT(f"[EXT] .OCT MaterialBundlePool -> name={base}, file={os.path.basename(mtbFilename)}")

    # Material pool (points to .mtl)
    oct_File.write('  <MaterialPool>\n')
    oct_File.write(f'    <Material name="{base}_material" file="{os.path.basename(mtlFilename)}"/>\n')
    oct_File.write('  </MaterialPool>\n')
    print(".MTL entry written")
    EXT(f"[EXT] .OCT MaterialPool -> name={base}_material, file={os.path.basename(mtlFilename)}")

    # Vertex buffer pool
    oct_File.write('  <VertexBufferPool>\n')
    oct_File.write(f'    <VertexBuffer name="{base}_vb" file="{os.path.basename(vbufFilename)}"/>\n')
    oct_File.write('  </VertexBufferPool>\n')
    print(".VBUF entry written")
    EXT(f"[EXT] .OCT VertexBufferPool -> name={base}_vb, file={os.path.basename(vbufFilename)}")

    # Index buffer pool
    oct_File.write('  <IndexBufferPool>\n')
    oct_File.write(f'    <IndexBuffer name="{base}_ib" file="{os.path.basename(ibufFilename)}"/>\n')
    oct_File.write('  </IndexBufferPool>\n')
    print(".IBUF entry written")
    EXT(f"[EXT] .OCT IndexBufferPool -> name={base}_ib, file={os.path.basename(ibufFilename)}")

    # Scene tree node pool
    oct_File.write('  <SceneTreeNodePool>\n')
    oct_File.write(f'    <SceneTreeNode name="{base}_node" />\n')
    oct_File.write('  </SceneTreeNodePool>\n')
    print("SceneTreeNodePool entry written")
    EXT(f"[EXT] .OCT SceneTreeNodePool -> name={base}_node")

    # Sub network pool
    oct_File.write('  <SubNetworkPool>\n')
    oct_File.write(f'    <SubNetwork name="{base}_network" />\n')
    oct_File.write('  </SubNetworkPool>\n')
    print("SubNetworkPool entry written")
    EXT(f"[EXT] .OCT SubNetworkPool -> name={base}_network")

    oct_File.write('</ObjectDatabase>\n')
    EXT(f"[EXT] .OCT root -> ObjectDatabase end")
